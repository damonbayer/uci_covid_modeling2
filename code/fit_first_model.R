...
n <- 2000
target <-
  tibble(frac_carrs = 1 - rbeta(n, 983, 2.7),
         frac_carrs_infec = rbeta(n, 41.3, 17.26),
         frac_infec_early = rbeta(n, 24.43, 27.02)) %>%
  mutate(EIeIp = frac_carrs * popsize,
         S = (1 - frac_carrs) * popsize,
  ...
  select(S, E, Ie, Ip) %>%
  round() %>%
  mutate(S = popsize - (E + Ie + Ip)) %>%
  as.matrix()

init_states <- c(S = popsize - sum(round(colMeans(target[,-1]))), round(colMeans(target[,-1])))

C <- optimize(f = function(C) sum(extraDistr::ddirmnom(x = target, size = popsize, alpha = init_states / C, log = T)), lower = 1, upper = 10000, maximum = T)$maximum

init_states <- c(S = popsize - sum(2 * round(colMeans(target[,-1]))), 2 * round(colMeans(target[,-1])))
init_states <- c(init_states, R = 0, D = 0)

# Else: do some fitting
