...
models_to_fit <-
  structure(list(start_date = structure(c(18490, 18504, 18525, 18539, 18560, 18574, 18588, 18598, 18612, 18619),
                                        class = "Date"),
                 end_date = structure(c(18525, 18539, 18560, 18574, 18595, 18609, 18623, 18633, 18647, 18654),
                                      class = "Date")),
            row.names = c(NA, -10L),
            class = c("tbl_df", "tbl", "data.frame")) %>%
  mutate(model_folder = str_c(start_date, end_date, sep = "_")) %>%
  mutate(model_path = path("code", "results", model_folder))
...
for (i in 10:nrow(models_to_fit)) {

  print(str_c("FITTING MODEL", i, "of", nrow(models_to_fit), sep = " "))
  first_day <- as_date(models_to_fit[[i, "start_date"]])
  last_day <-  as_date(models_to_fit[[i, "end_date"]])
  ...
  prior_model_fit <- read_rds(path(prior_model$model_path, city_folder_name, "original", ext = "rds"))
  ...
  results_folder <- path(models_to_fit[[i, "model_path"]], city_folder_name)
  dir_create(results_folder)
  write_rds(multi_chain_stem_fit, path(models_to_fit[[i, "model_path"]], city_folder_name, "original", ext = "rds"))
}
