---
title: "LOCATION_HERE, CA COVID Situation Report START_DATE_HERE - END_DATE_HERE"
...
results_folder <- RESULTS_FOLDER_HERE
city_name <- NA
location_name <- NA
...
## LOCATION_HERE, CA COVID-19 Situation Report, `r format(last_day + 5, "%B %e, %Y")`
...
Reff_intervals <- 
  ...
  left_join(prev_models %>% 
              mutate(time_date_conversion = forecast_obj %>%
                       map("data") %>%
                       map(~select(., time, date = end_date))) %>%
