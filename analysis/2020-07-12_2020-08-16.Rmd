---
title: "Orange County, CA COVID Situation Report Jul 12, 2020 - Aug 16, 2020"
...
results_folder <- "code/results/2020-07-12_2020-08-16"
...
## Orange County, CA COVID-19 Situation Report, `r format(last_day + 8, "%B %e, %Y")`
...
ifr_est <- ifr_R0_samples %>% filter(end_date == last_day) %>% pull(ifr) %>% quantile(c(0.025, 0.975))
R0_est <- ifr_R0_samples %>% filter(end_date == last_day) %>% pull(R0) %>% quantile(c(0.025, 0.975))
Reff_est <- Reff_intervals %>% drop_na() %>% filter(date == last_day, .width == 0.95) %>% select(".lower", ".upper") %>% pivot_longer(everything()) %>% deframe()
```
